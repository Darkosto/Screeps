...
module.exports.loop = function () {
  ...
  if (workers.length < 6) {
    var newName = Game.spawns["Spawn1"].spawnCreep(
      [WORK, CARRY, MOVE],
      "worker" + (Math.floor(Math.random() * 65534) + 1),
      {
        memory: {
          role: "worker",
        },
  ...
  ////
  var midworkers = _.filter(
    Game.creeps,
    (creep) => creep.memory.role == "midworker"
  );

  if (midworkers.length < 1) {
    var newName = Game.spawns["Spawn1"].spawnCreep(
      [WORK, CARRY, MOVE, MOVE],
      "MidPointWorker" + (Math.floor(Math.random() * 65534) + 1),
      {
        memory: {
          role: "midworker",
        },
  ...
  // Construction
  ////
  var constructors = _.filter(
    Game.creeps,
    (creep) => creep.memory.role == "constructor"
  );
  ...
  if (constructors.length < 2 && ConstructionSite.length > 0) {
    var newName = Game.spawns["Spawn1"].spawnCreep(
      [WORK, CARRY, MOVE, MOVE],
      "constructor" + (Math.floor(Math.random() * 65534) + 1),
      {
        memory: {
          role: "constructor",
        },
  ...
  if (repairs.length < 2) {
    var newName = Game.spawns["Spawn1"].spawnCreep(
      [WORK, CARRY, MOVE],
      "repairer" + (Math.floor(Math.random() * 65534) + 1),
      {
        memory: {
          role: "repairer",
        },
  ...
  if (scouts.length < 1) {
    var newName = Game.spawns["Spawn1"].spawnCreep(
      [MOVE, CLAIM, MOVE],
      "scout" + (Math.floor(Math.random() * 65534) + 1),
      {
        memory: {
          role: "scout",
        },
